...
function secure_document($elt, $cols)
{
    ...
    // Vérifier si la clé 'synopsis' existe dans le tableau $elt
    if (!isset($elt['synopsis'])) {
        $elt['synopsis'] = '';  // Fournir une valeur par défaut si la clé n'existe pas
    }

    // Utiliser la valeur de 'synopsis' comme argument pour nl2br()
    $synopsis = nl2br($elt['synopsis']);

    // Vérifier si la clé 'id_tmdb' existe dans le tableau $elt
    if (!isset($elt['id_tmdb'])) {
        $elt['id_tmdb'] = '';  // Fournir une valeur par défaut si la clé n'existe pas
    }
